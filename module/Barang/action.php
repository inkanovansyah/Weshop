<?php
    ...
    $update_gambar = "";
		...
		//if ini akan mengecek apakah file variabel ini tidak kosong jika benar tidak kosong maka kita akan memberikan proses upload
        if(!empty($_FILES["file"]["name"])){
            $nama_file = $_FILES["file"]["name"]; 
            move_uploaded_file($_FILES["file"]["tmp_name"], "../../images/barang/".$nama_file);
        //fungis jika user tidak melakukan upload maka tidak terjadi error
            $update_gambar = ",gambar='$nama_file'";
        }
        //web dasar untuk memasukan image kedalam server
        if($button == "Add"){
